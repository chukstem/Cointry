...
class BuyScreen extends StatefulWidget {
  static String routeName = "/buy-crypto";
  CryptoModel from, to;
  MarketsModel market;
  BuyScreen({Key? key, required this.from, required this.to, required this.market}) : super(key: key);
    ...
    return Scaffold(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            backAppbar(context, "${Language.buy} ${widget.from.networkModel[0].currency_symbol}"),
            SizedBox(
              height: 30,
            ),
            Container(
              margin: EdgeInsets.only(top: 10, right: 10, left: 40),
              child: Text("${Language.pay}: ${widget.to.networkModel[0].currency_symbol} (${widget.to.balance})", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14),),
            ),
            Container(
            ...
            Container(
              ...
              child: Text("${Language.receive}: ${widget.from.networkModel[0].currency_symbol} (${widget.from.balance})", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14),),
            ),
            Container(
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () {
                    if(double.parse(CryptoAmountController.text.replaceAll(",", ""))>0){
                      _confirm(context, format(CryptoAmountController.text, widget.to.networkModel.first.decimals));
                    }
                  },
                )),
            SizedBox(height: 10,),
