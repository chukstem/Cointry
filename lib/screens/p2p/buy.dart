...
class Buy extends StatefulWidget {
  ...
  Buy({Key? key, required this.clearCallback}) : super(key: key);
...
class _BuyState extends State<Buy> with SingleTickerProviderStateMixin {
  ...
  cachedList() async {
    List<P2PModel> iList = await getP2PCached(currency_id, "1");
    setState(() {
      if (iList.isNotEmpty) {
        loading = false;
        tList = iList;
  ...
  fetch() async {
    try {
      List<P2PModel> iList = await getP2P(http.Client(), "0", currency_id, amount, bank_id, "1", sort);
      setState(() {
        tList = iList;
      });
    } catch (e) {}
  ...
  addItems() async {
    ...
    try {
      List<P2PModel> iList = await getP2P(http.Client(), "$length", currency_id, amount, bank_id, "1", sort);
      setState(() {
        tList.addAll(iList);
        if (iList.isEmpty) {
          stopRefresh = true;
                              ...
                              Container(
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    ...
                                    InkWell(
                                      onTap: () async {
                                        final result = await Navigator.push(context, MaterialPageRoute<dynamic>(builder: (BuildContext context) => P2PBuyScreen(obj: tList![i],)));
                                        setState(() {
                                          fetch();
                                        });
                                      },
